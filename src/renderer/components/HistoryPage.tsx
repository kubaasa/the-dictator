  ...
  return (
    <div className="border-b border-neutral-800 last:border-b-0">
      ...
      <button
        ...
        className="w-full flex items-center gap-3 px-4 py-3 text-left hover:bg-red-950/30 transition-colors cursor-pointer"
      >
        ...
        {modeBadge && (
          <span className="font-mono text-[9px] font-bold tracking-wider text-neutral-500 border border-neutral-700 rounded px-1.5 py-0.5 shrink-0">
            {modeBadge}
          </span>
        )}
        <span className="text-sm text-neutral-300 truncate flex-1">{preview}</span>
        {entry.wordCount > 0 && (
          <span className="font-mono text-[10px] text-neutral-600 shrink-0">{entry.wordCount}w</span>
        )}
      </button>
      ...
      >
        ...
        <div className="mx-4 mb-3 rounded-lg border border-red-900/30 bg-[#141414] overflow-hidden">
          ...
          {/* Delete error */}
          {deleteError && (
            <div className="px-3 py-1.5 text-xs text-red-400 bg-red-950/30" role="alert">
              {deleteError}
            </div>
          )}
          ...
          <div className="flex items-center justify-center px-3 py-2">
            {confirmingDelete ? (
              <div className="flex items-center gap-2">
                <span className="font-mono text-[11px] text-neutral-400">Delete?</span>
                <button
                  onClick={handleDeleteConfirm}
                  disabled={isDeleting}
                  className="rounded-md px-3 py-1.5 font-mono text-[11px] font-semibold uppercase tracking-wider text-red-400 border border-red-800 hover:bg-red-950/50 transition-colors disabled:opacity-50 disabled:cursor-wait"
                >
                ...
                <button
                  ...
                  disabled={isDeleting}
                  className="rounded-md px-3 py-1.5 font-mono text-[11px] font-semibold uppercase tracking-wider text-neutral-400 border border-neutral-700 hover:border-neutral-500 hover:text-neutral-200 transition-colors disabled:opacity-50"
                >
                  No
                </button>
              </div>
          ...
          <div className="flex flex-col items-center justify-center h-full gap-3 text-neutral-600">
            {loadTimedOut ? (
              <>
                ...
                <button onClick={() => loadAll()} className="font-mono text-[11px] font-semibold uppercase tracking-[0.25em] text-neutral-500 hover:text-neutral-300 underline">
                  Retry
                </button>
              </>
            ) : (
          ...
          <div className="flex flex-col items-center justify-center h-full gap-3 text-neutral-600">
            {loadError ? (
              <>
                ...
                <p className="text-xs text-neutral-600 max-w-xs text-center">{loadError}</p>
              ...
              <>
                ...
                <p className="text-xs text-neutral-600">Start recording to see history here</p>
              </>
            )}
          </div>
        ) : (
          groups.map(({ label, entries }) => (
            <div key={label}>
              <div className="px-4 py-2 sticky top-0 z-20 bg-[#0A0A0A]">
                <span className="font-mono text-[11px] font-semibold tracking-[0.25em] text-red-700">{label}</span>
